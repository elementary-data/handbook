...
These are the best practices for how we currently implement our culture principals. Use these as in day to day work as a framework for decision making.
...
We don’t obsess about goals, but it’s an important tool for learning. Sometimes you learn that your goal is wrong, or that you don’t measure it correctly. Other times you understand you didn’t make the right decisions, and adapt. And sometimes you win, and you should celebrate!

### We experiment and iterate

We are humble enough to admit that it’s really hard to know what will make an impact. We see every initiative we make as an experiment, an assumption we need to test. A good experiment is fast, and has clear results. This means that we try to reduce the scope to the minimum that could help us determine if our assumption is correct. If it is, you can do another iteration (which is a new experiment with new assumptions).
...
This is pretty straight forward - we set goals, we experiment and try to make an impact, we leverage data to understand what worked and what didn’t. We actively think about what data do we new need to answer our questions, and which new learnings we can produce from it.

### Default to action

As a small startup, we won’t win with resources. We will win with speed and flexibility. When you need to make a decision on how or if to do something, ask yourself if:
...
Writing enables us to work async, not relay on our memory, include new team members in past discussions, and avoid [unnecessary meetings](https://www.youtube.com/watch?v=Yerar4Xa_iE). If you want to consult about a feature implementation, sync on open issues in the community, or share creative ideas on how we can conquer the world - do it in writing.
...
We are not always on the same time zone and place. Even when we are, we should be able to deeply focus on tasks without interruptions. This means that by default we don’t expect each other to answer messages immediately, just in a timely fashion during the work day.
